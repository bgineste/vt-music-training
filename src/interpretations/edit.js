...
//		let aliste = liste.split('|')
	...
	const onChangeCaract = event => {
		console.log("****")
		let caract = event.target.value
		let nom = event.target.name
		props.caracteristiques.setAttributes( {properties: ['interprete', caract]} )
				...
				<>
					{props.attributes.caracteristiques ? (
						...
						console.log(props.attributes.caracteristiques),
								...
								<label>Interprète<br />
								<input type='text' name='interprete' value={props.attributes.caracteristiques.interprete}  onChange={ onChangeCaract } placeholder={ __('Write a text!', 'vt-music-training' ) } />
								</label> 
